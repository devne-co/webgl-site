...
void main(void){
 ...
 //  float f = random(vec2(gl_FragCoord.y,uTime));
  // gl_FragColor = vec4(1,1,uTime,1);
   ...
   vec4 color = vec4(texture2D(tDiffuse,vec2(newPos.x + 0.002,newPos.y)).r + a,texture2D(tDiffuse,vec2(newPos.x + 0.004,newPos.y)).g + a,texture2D(tDiffuse,vec2(newPos.x + 0.006,newPos.y)).b + a,1.0);
